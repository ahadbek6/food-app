import { useState } from 'react';
import Image1 from "../assets/images/ProductManagement.svg"

const SettingsPage = () => {
  const [activeSettingContent, setActiveSettingContent] = useState("products management");
  
  return (
    <div className="w-full relative">
      <div className="ps-[133px] p-6 w-full  ">
        <h1 className="w-full mb-6 text-[28px] font-semibold leading-[39.2px]">
          Settings
        </h1>
        <div className="flex">
          <div className="bg-dark_bg_2 w-[275px] rounded-lg">
            <ul>
              <li className={`cursor-pointer py-6 ps-6 ${activeSettingContent === "appereance" ? "text-primary bg-[#EA7C6942]" : "" }`} onClick={() => setActiveSettingContent("appereance")}>
              <div className={`flex gap-2 ${activeSettingContent === "appereance" ? "border-r-4 border-r-primary" : "border-r-4 border-r-transparent " }`} >
                  <span className="translate-y-0.5">
                    <span className="translate-y-0 5">
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M4.32223 1.87682C5.49376 1.49998 6.76145 1.63559 7.82216 2.23745L7.99776 2.34282L8.17651 2.23686C9.18215 1.66749 10.3751 1.51535 11.4977 1.82411L11.6748 1.87686C14.1233 2.66516 15.2196 5.33863 14.3959 7.89673C13.983 9.0867 13.3057 10.1656 12.4118 11.0549C11.1953 12.2363 9.85429 13.2796 8.41471 14.1648L8.26188 14.2598C8.1067 14.3562 7.91057 14.3579 7.75379 14.264L7.59008 14.166C6.14852 13.2796 4.80751 12.2363 3.58686 11.0508C2.69706 10.1656 2.01981 9.0867 1.60338 7.8863C0.782739 5.33708 1.87451 2.66488 4.32223 1.87682ZM7.58326 3.23614C6.71387 2.64189 5.62237 2.48222 4.62119 2.80426C2.72514 3.4147 1.86555 5.51863 2.52821 7.57753C2.89267 8.62779 3.4902 9.57968 4.27071 10.3562C5.436 11.4879 6.72049 12.4872 8.09619 13.3332L8.00101 13.2732L8.31291 13.077C9.40186 12.3739 10.4275 11.5761 11.3759 10.6947L11.7279 10.3603C12.5126 9.57968 13.1101 8.62779 13.4711 7.58795C14.1366 5.5206 13.2733 3.41519 11.3762 2.80442C10.325 2.46717 9.1775 2.66034 8.29082 3.32413C8.12059 3.45157 7.88731 3.45404 7.71441 3.33021L7.58326 3.23614ZM10.5803 4.32245C11.4613 4.60363 12.086 5.39235 12.164 6.31898C12.1865 6.58709 11.9873 6.82272 11.719 6.84525C11.4506 6.86779 11.2148 6.66871 11.1922 6.4006C11.1472 5.86531 10.7878 5.41153 10.2836 5.25061C10.0271 5.16873 9.88557 4.89459 9.96751 4.63829C10.0494 4.38198 10.3238 4.24058 10.5803 4.32245Z"
                          fill={`${activeSettingContent === "appereance" ? "#EA7C69" : "#ABBBC2" }`}
                        />
                      </svg>
                    </span>
                  </span>
                  <div className="">
                    <h4 className="text-sm leading-[18.20px] mb-0.5">
                      Appereance
                    </h4>
                    <p className="text-bg_gray text-xs">
                      Dark and Light mode, Font size
                    </p>
                  </div>
                </div>
              </li>
              <li className={`cursor-pointer py-6 ps-6 ${activeSettingContent === "your restaurant" ? "text-primary bg-[#EA7C6942]" : "" }`} onClick={() => setActiveSettingContent("your restaurant")}>
                <div className={`flex gap-2  ${activeSettingContent === "your restaurant" ? "border-r-4 border-r-primary" : "border-r-4 border-r-transparent" }`} >
                  <span className="translate-y-0 5">
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 16 16"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M14.6425 4.51624L13.6902 1.6591C13.6254 1.46454 13.4433 1.33332 13.2383 1.33337H2.76207C2.55702 1.33332 2.37496 1.46454 2.31018 1.6591L1.35778 4.51624C1.3431 4.56072 1.3349 4.60706 1.3335 4.65385C1.3335 4.65862 1.3335 4.66194 1.3335 4.66671V5.14291C1.33523 5.88901 1.68805 6.59079 2.28587 7.03719C2.28587 7.04101 2.28587 7.04386 2.28587 7.04766V13.2381C2.28587 14.0271 2.92547 14.6667 3.71444 14.6667H12.2859C13.0748 14.6667 13.7144 14.0271 13.7144 13.2381V7.04766C13.7144 7.04383 13.7144 7.04099 13.7144 7.03719C14.3123 6.59079 14.6651 5.88901 14.6668 5.14291V4.66671C14.6668 4.66194 14.6668 4.65862 14.6668 4.65385C14.6654 4.60703 14.6572 4.56069 14.6425 4.51624ZM3.1335 2.28574H12.8949L13.5302 4.19051H2.46683L3.1335 2.28574ZM8.95255 13.7143H7.04778V9.9048H8.95255V13.7143ZM12.7621 13.2381C12.7621 13.5011 12.5489 13.7143 12.2859 13.7143H9.90493V9.42862C9.90493 9.16562 9.69173 8.95242 9.42873 8.95242H6.57159C6.30858 8.95242 6.09539 9.16562 6.09539 9.42862V13.7143H3.71444C3.45144 13.7143 3.23825 13.5011 3.23825 13.2381V7.47623C3.90652 7.61342 4.60144 7.45393 5.14301 7.03909C5.99307 7.66342 7.1501 7.66342 8.00016 7.03909C8.85021 7.66342 10.0072 7.66342 10.8573 7.03909C11.3989 7.45396 12.0938 7.61342 12.7621 7.47623V13.2381ZM12.2859 6.57146C11.8741 6.5712 11.483 6.39057 11.2159 6.07718C11.0428 5.87916 10.742 5.8589 10.544 6.03195C10.5279 6.04598 10.5128 6.06111 10.4987 6.07718C9.96924 6.66814 9.06098 6.71794 8.47002 6.18848C8.43093 6.15343 8.39377 6.11629 8.35872 6.07718C8.17237 5.87916 7.86076 5.86967 7.66271 6.05605C7.65546 6.06289 7.6484 6.06992 7.64159 6.07718C7.11213 6.66814 6.20384 6.71794 5.61288 6.18848C5.57376 6.15343 5.53663 6.11629 5.50158 6.07718C5.31522 5.87916 5.00359 5.86967 4.80557 6.05605C4.79832 6.06289 4.79126 6.06992 4.78445 6.07718C4.51729 6.39057 4.12627 6.5712 3.71444 6.57146C2.92547 6.57146 2.28587 5.93186 2.28587 5.14289H13.7144C13.7144 5.93186 13.0748 6.57146 12.2859 6.57146Z"
                        fill={`${activeSettingContent === "your restaurant" ? "#EA7C69" : "#ABBBC2" }`}
                      />
                    </svg>
                  </span>
                  <div className="">
                    <h4 className="text-sm leading-[18.20px] mb-0.5">
                    Your Restaurant
                    </h4>
                    <p className="text-bg_gray text-xs ">
                      Dark and Light mode, Font size
                    </p>
                </div>
                </div>
              </li>
              <li className={`cursor-pointer py-6 ps-6 ${activeSettingContent === "products management" ? "text-primary bg-[#EA7C6942]" : "" }`} onClick={() => setActiveSettingContent("products management")}>
                <div className={`flex gap-2 ${activeSettingContent === "products management" ? "border-r-4 border-r-primary" : "border-r-4 border-r-transparent" }`}>
                    <span className="translate-y-0 5">
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M9.30082 1.81105L9.42237 1.92438L9.90153 2.40355C10.0661 2.56736 10.282 2.66836 10.5117 2.69105L10.6108 2.69594H11.2962C12.3654 2.69594 13.2397 3.53205 13.3007 4.58597L13.3042 4.70394V5.3886C13.3042 5.6218 13.3846 5.84598 13.5296 6.02424L13.5957 6.09772L14.0749 6.57692C14.8292 7.32785 14.863 8.52871 14.1719 9.32009L14.0817 9.41682L13.5961 9.9024C13.4328 10.0661 13.3318 10.2822 13.309 10.5116L13.3042 10.6106V11.2966C13.3042 12.3658 12.4681 13.2395 11.4141 13.3005L11.2962 13.3039H10.6108C10.3785 13.3039 10.154 13.3847 9.97563 13.5301L9.90212 13.5964L9.42357 14.0743C8.67344 14.8295 7.47171 14.863 6.68003 14.1717L6.57726 14.0755L6.09919 13.5974C5.93359 13.433 5.7173 13.3316 5.4883 13.3088L5.38948 13.3039H4.70348C3.63473 13.3039 2.76063 12.4683 2.69956 11.4145L2.69615 11.2966V10.6106C2.69615 10.3786 2.61529 10.1539 2.4699 9.9759L2.40369 9.90258L1.9256 9.42382C1.17055 8.67303 1.13706 7.47128 1.8284 6.68038L1.9186 6.58372L2.40268 6.09897C2.56731 5.93317 2.66853 5.71729 2.69126 5.48772L2.69615 5.3886V4.70394L2.69956 4.58598C2.75837 3.5712 3.57115 2.75818 4.58557 2.69935L4.70348 2.69594H5.38948C5.6218 2.69594 5.84632 2.61535 6.02435 2.47056L6.09769 2.40463L6.57646 1.92518C7.32737 1.17087 8.52899 1.13766 9.30082 1.81105ZM7.36109 2.56179L7.28462 2.63124L6.80378 3.11274C6.45895 3.45513 6.00287 3.65985 5.52137 3.69159L5.38948 3.69594H4.70348C4.17844 3.69594 3.74663 4.0987 3.70027 4.61222L3.69615 4.70394V5.3886C3.69615 5.87471 3.51974 6.34214 3.20155 6.70752L3.11128 6.80458L2.6265 7.29002C2.25863 7.65957 2.23721 8.24335 2.56231 8.63938L2.63195 8.71596L3.11146 9.19614C3.45439 9.53988 3.65992 9.99677 3.69179 10.4786L3.69615 10.6106V11.2966C3.69615 11.822 4.09862 12.2535 4.61183 12.2998L4.70348 12.3039H5.38948C5.87468 12.3039 6.34251 12.4805 6.70797 12.7988L6.80504 12.889L7.28725 13.3713C7.65939 13.7413 8.24333 13.7629 8.639 13.4378L8.71551 13.3681L9.1961 12.8882C9.54058 12.5454 9.99687 12.3401 10.4788 12.3083L10.6108 12.3039H11.2962C11.8219 12.3039 12.2537 11.9017 12.3 11.3883L12.3042 11.2966V10.6106C12.3042 10.1247 12.4807 9.65604 12.7985 9.29226L12.8886 9.19571L13.3736 8.71072C13.7415 8.34075 13.7628 7.75726 13.4381 7.36138L13.3686 7.28482L12.8873 6.80357C12.5447 6.45852 12.3402 6.0029 12.3085 5.52071L12.3042 5.3886V4.70394C12.3042 4.17833 11.9014 3.74642 11.3879 3.70006L11.2962 3.69594H10.6108C10.1247 3.69594 9.65667 3.51968 9.29207 3.2017L9.19526 3.11149L8.73664 2.65164L8.71007 2.62696C8.34055 2.25911 7.75664 2.23758 7.36109 2.56179ZM10.0644 9.36167C10.1111 9.40834 10.1511 9.46167 10.1778 9.52167C10.1978 9.58167 10.2111 9.64834 10.2111 9.715C10.2111 9.78234 10.1978 9.84167 10.1778 9.90167C10.1511 9.96167 10.1111 10.0217 10.0644 10.0683C9.97108 10.1617 9.84442 10.215 9.71108 10.215C9.57775 10.215 9.45108 10.1617 9.35775 10.0683C9.31108 10.0217 9.27775 9.96167 9.25108 9.90167C9.22442 9.84167 9.21108 9.78234 9.21108 9.715C9.21108 9.64834 9.22442 9.58167 9.25108 9.52167C9.27775 9.46167 9.31108 9.40834 9.35775 9.36167C9.54442 9.175 9.87775 9.175 10.0644 9.36167ZM10.0668 5.93285C10.2443 6.11036 10.2605 6.38814 10.1152 6.58388L10.0668 6.63996L6.64017 10.0666C6.44491 10.2619 6.12833 10.2619 5.93306 10.0666C5.75555 9.88911 5.73942 9.61133 5.88465 9.41559L5.93306 9.35952L9.35973 5.93285C9.55499 5.73759 9.87158 5.73759 10.0668 5.93285ZM6.63768 5.93527C6.73102 6.02794 6.78435 6.15527 6.78435 6.2886C6.78435 6.35527 6.77768 6.4146 6.75102 6.47527C6.72435 6.53527 6.68435 6.5886 6.63768 6.64194C6.59102 6.6886 6.53768 6.72127 6.47768 6.74794C6.41768 6.7746 6.35102 6.7886 6.28435 6.7886C6.21768 6.7886 6.15768 6.7746 6.09768 6.74794C6.03768 6.72127 5.97768 6.6886 5.93102 6.64194C5.88435 6.5886 5.85102 6.53527 5.82435 6.47527C5.79768 6.4146 5.78435 6.35527 5.78435 6.2886C5.78435 6.22127 5.79768 6.15527 5.82435 6.09527C5.85102 6.03527 5.88435 5.97527 5.93102 5.93527C6.12435 5.74794 6.45102 5.74794 6.63768 5.93527Z"
                          fill={`${activeSettingContent === "products management" ? "#EA7C69" : "#ABBBC2" }`}
                        />
                      </svg>
                    </span>
                  <div>
                    <h4 className="text-sm leading-[18.20px] mb-0.5">
                    Products Management
                    </h4>
                    <p className="text-bg_gray text-xs">
                    Manage your product, pricing, etc
                    </p>
                  </div>
                </div>
              </li>
              <li className={`cursor-pointer py-6 ps-6 ${activeSettingContent === "notifications" ? "text-primary bg-[#EA7C6942]" : "" }`} onClick={() => setActiveSettingContent("notifications")}>
                <div className={`flex gap-2 ${activeSettingContent === "notifications" ? "border-r-4 border-r-primary" : "border-r-4 border-r-transparent " }`}>
                  <span className="translate-y-0 5">
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 16 16"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.7388 13.1271C9.93652 13.2881 9.96786 13.5809 9.80879 13.7811C9.6912 13.929 9.55532 14.0615 9.41131 14.1701C8.92623 14.5507 8.3095 14.7233 7.69703 14.6503C7.08401 14.5773 6.52483 14.2642 6.14322 13.7793C5.98509 13.5784 6.0178 13.2858 6.21628 13.1257C6.41476 12.9657 6.70384 12.9988 6.86197 13.1997C7.09211 13.4921 7.43129 13.682 7.80443 13.7265C8.1781 13.771 8.55399 13.6658 8.8552 13.4295C8.94402 13.3625 9.02384 13.2847 9.09277 13.198C9.25184 12.9978 9.54108 12.9661 9.7388 13.1271ZM3.43491 5.99987L3.44312 5.60265C3.44899 5.43274 3.45807 5.27503 3.47139 5.12068C3.67119 2.96308 5.73059 1.33337 7.97669 1.33337H8.02424C10.2691 1.33337 12.3291 2.96194 12.5357 5.12362C12.5476 5.26103 12.5527 5.38309 12.5542 5.55475L12.5548 6.07992C12.5553 6.12776 12.5561 6.16983 12.5574 6.20906L12.5635 6.33989L12.5932 6.46463C12.687 6.81206 12.8519 7.13687 13.0777 7.41885L13.1781 7.53701L13.2193 7.58982C13.49 7.9945 13.6447 8.46538 13.6668 8.97253L13.6662 9.2414C13.6473 9.84202 13.4268 10.4221 13.0227 10.9055C12.4876 11.4734 11.7624 11.8263 10.9917 11.8972C9.00149 12.1133 6.99349 12.1133 5.00881 11.8978C4.2328 11.8231 3.50979 11.4713 2.95125 10.8816C2.53938 10.3795 2.32044 9.74723 2.3341 9.10833L2.33466 8.94949C2.35846 8.46372 2.51219 7.99327 2.77974 7.58684L2.82452 7.52923C3.13699 7.18666 3.35062 6.76741 3.44332 6.31541L3.43329 6.35601L3.43491 5.99987ZM8.02424 2.26361H7.97669C6.17784 2.26361 4.53885 3.56062 4.38661 5.20456C4.37882 5.29489 4.3726 5.38809 4.36775 5.48617L4.35704 5.7971L4.35269 6.40995L4.3431 6.50449C4.21645 7.12205 3.92502 7.69399 3.49969 8.16028L3.53254 8.12221L3.47379 8.21943C3.36375 8.41771 3.29276 8.63304 3.26471 8.84553L3.25306 8.97253L3.25296 9.11832C3.24395 9.54287 3.38741 9.95713 3.63596 10.2621C4.01937 10.6657 4.53805 10.918 5.10128 10.9723C7.02635 11.1813 8.96863 11.1813 10.9011 10.9715C11.4582 10.9202 11.9765 10.668 12.3391 10.2845C12.6135 9.9554 12.7585 9.53981 12.7479 9.10243L12.7484 8.99347C12.7343 8.67908 12.6342 8.37421 12.4584 8.1115L12.4686 8.12841L12.3766 8.02061C12.0597 7.62944 11.8295 7.17571 11.7013 6.68935L11.658 6.50549L11.6494 6.44371C11.643 6.35346 11.6392 6.27325 11.6372 6.1768L11.6354 5.57371C11.6341 5.42034 11.63 5.3171 11.6207 5.20888C11.4631 3.56054 9.82258 2.26361 8.02424 2.26361Z"
                        fill={`${activeSettingContent === "notifications" ? "#EA7C69" : "#ABBBC2" }`}
                      />
                    </svg>
                  </span>
                  <div className="">
                    <h4 className="text-sm leading-[18.20px] mb-0.5">
                    Notifications
                    </h4>
                    <p className="text-bg_gray text-xs ">
                      Dark and Light mode, Font size
                    </p>
                  </div>
                </div>
              </li>
              <li className={`cursor-pointer py-6 ps-6 ${activeSettingContent === "Security" ? "text-primary bg-[#EA7C6942]" : "" }`} onClick={() => setActiveSettingContent("Security")}>
                <div className={`flex gap-2 ${activeSettingContent === "Security" ? "border-r-4 border-r-primary" : "border-r-4 border-r-transparent" }`}>
                  <span className="translate-y-0 5">
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 16 16"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.00674 1.3334C9.54965 1.3334 10.9053 2.29938 11.4021 3.71113C11.494 3.97207 11.3534 4.25684 11.0882 4.34719C10.8229 4.43754 10.5335 4.29925 10.4416 4.03831C10.0854 3.02623 9.11313 2.3334 8.00472 2.3334C6.62673 2.32803 5.49704 3.38767 5.42213 4.72388L5.4182 4.85403L5.41816 5.75071L10.589 5.75094C12.2402 5.75094 13.5878 7.03051 13.6635 8.63696L13.6668 8.77953V11.6381C13.6668 13.3108 12.2888 14.6667 10.589 14.6667H5.41128C3.71152 14.6667 2.3335 13.3108 2.3335 11.6381V8.77953C2.3335 7.45489 3.19775 6.32888 4.40163 5.91767L4.40175 4.85184C4.41044 2.90059 6.02518 1.3257 8.00674 1.3334ZM10.589 6.75091H5.41128C4.27293 6.75091 3.34994 7.65912 3.34994 8.77953V11.6381C3.34994 12.7585 4.27293 13.6667 5.41128 13.6667H10.589C11.7274 13.6667 12.6504 12.7585 12.6504 11.6381V8.77953C12.6504 7.65912 11.7274 6.75091 10.589 6.75091ZM8.0003 8.96832C8.25759 8.96832 8.47023 9.15642 8.50388 9.40047L8.50852 9.46831V10.9496C8.50852 11.2257 8.28098 11.4496 8.0003 11.4496C7.74301 11.4496 7.53037 11.2615 7.49672 11.0175L7.49208 10.9496V9.46831C7.49208 9.19217 7.71962 8.96832 8.0003 8.96832Z"
                        fill={`${activeSettingContent === "Security" ? "#EA7C69" : "#ABBBC2" }`}
                      />
                    </svg>
                  </span>
                  <div className="">
                    <h4 className="text-sm leading-[18.20px] mb-0.5">
                    Security
                    </h4>
                    <p className="text-bg_gray text-xs ">
                    Configure Password, PIN, etc
                    </p>
                  </div>
                </div>
              </li>
              <li className={`cursor-pointer py-6 ps-6 ${activeSettingContent === "security" ? "text-primary bg-[#EA7C6942]" : "" }`} onClick={() => setActiveSettingContent("security")}>
                <div className={`flex gap-2 ${activeSettingContent === "security" ? "border-r-4 border-r-primary" : "border-r-4 border-r-transparent " }`}>
                  <span className="translate-y-0 5">
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 16 16"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.00674 1.3334C9.54965 1.3334 10.9053 2.29938 11.4021 3.71113C11.494 3.97207 11.3534 4.25684 11.0882 4.34719C10.8229 4.43754 10.5335 4.29925 10.4416 4.03831C10.0854 3.02623 9.11313 2.3334 8.00472 2.3334C6.62673 2.32803 5.49704 3.38767 5.42213 4.72388L5.4182 4.85403L5.41816 5.75071L10.589 5.75094C12.2402 5.75094 13.5878 7.03051 13.6635 8.63696L13.6668 8.77953V11.6381C13.6668 13.3108 12.2888 14.6667 10.589 14.6667H5.41128C3.71152 14.6667 2.3335 13.3108 2.3335 11.6381V8.77953C2.3335 7.45489 3.19775 6.32888 4.40163 5.91767L4.40175 4.85184C4.41044 2.90059 6.02518 1.3257 8.00674 1.3334ZM10.589 6.75091H5.41128C4.27293 6.75091 3.34994 7.65912 3.34994 8.77953V11.6381C3.34994 12.7585 4.27293 13.6667 5.41128 13.6667H10.589C11.7274 13.6667 12.6504 12.7585 12.6504 11.6381V8.77953C12.6504 7.65912 11.7274 6.75091 10.589 6.75091ZM8.0003 8.96832C8.25759 8.96832 8.47023 9.15642 8.50388 9.40047L8.50852 9.46831V10.9496C8.50852 11.2257 8.28098 11.4496 8.0003 11.4496C7.74301 11.4496 7.53037 11.2615 7.49672 11.0175L7.49208 10.9496V9.46831C7.49208 9.19217 7.71962 8.96832 8.0003 8.96832Z"
                        fill={`${activeSettingContent === "security" ? "#EA7C69" : "#ABBBC2" }`}
                      />
                    </svg>
                  </span>
                  <div className="">
                    <h4 className="text-sm leading-[18.20px] mb-0.5">
                    Security
                    </h4>
                    <p className="text-bg_gray text-xs ">
                    Configure Password, PIN, etc
                    </p>
                  </div>
                </div>
              </li>
              <li className={`cursor-pointer py-6 ps-6 ${activeSettingContent === "about us" ? "text-primary bg-[#EA7C6942]" : "" }`} onClick={() => setActiveSettingContent("about us")}>
                <div className={`flex gap-2 ${activeSettingContent === "about us" ? "border-r-primary border-r-4" : "border-r-4 border-r-transparent" }`} >
                  <span className="translate-y-0 5">
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 14 14"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M7.00016 0.333496C10.6821 0.333496 13.6668 3.31846 13.6668 7.00016C13.6668 10.6819 10.6821 13.6668 7.00016 13.6668C3.31869 13.6668 0.333496 10.6816 0.333496 7.00016C0.333496 3.31869 3.31869 0.333496 7.00016 0.333496ZM7.00016 1.3335C3.87097 1.3335 1.3335 3.87097 1.3335 7.00016C1.3335 10.1294 3.87097 12.6668 7.00016 12.6668C10.1298 12.6668 12.6668 10.1296 12.6668 7.00016C12.6668 3.87073 10.1298 1.3335 7.00016 1.3335ZM7.00356 9.03076C7.27971 9.03076 7.50356 9.25462 7.50356 9.53076C7.50356 9.78389 7.31546 9.99309 7.07141 10.0262L6.9969 10.0308C6.72075 10.0308 6.4969 9.8069 6.4969 9.53076C6.4969 9.27763 6.685 9.06843 6.92905 9.03533L7.00356 9.03076ZM6.9969 3.96956C7.25003 3.96956 7.45922 4.15767 7.49233 4.40172L7.4969 4.46956V7.41556C7.4969 7.6917 7.27304 7.91556 6.9969 7.91556C6.74377 7.91556 6.53457 7.72746 6.50146 7.48341L6.4969 7.41556V4.46956C6.4969 4.19342 6.72075 3.96956 6.9969 3.96956Z"
                        fill={`${activeSettingContent === "about us" ? "#EA7C69" : "#ABBBC2" }`}
                      />
                    </svg>
                  </span>
                  <div className="">
                    <h4 className="text-sm leading-[18.20px] mb-0.5">
                    About Us
                    </h4>
                    <p className="text-bg_gray text-xs ">
                    Find out more about Posly
                    </p>
                  </div>
                </div>
              </li>
            </ul>
          </div>
        </div>
      </div>
      <div className="absolute  w-[980px] h-[623px] top-[87px] right-6 bottom-6">
          <img src={Image1} alt="" />
        </div>
    </div>
  );
}

export default SettingsPage;